...
# Country dropdown includes All countries.
...
# Country-specific or all-country trend data
...
with row1_col1:
    ...
    st.markdown(
        '<p class="small-note">Population-level association only; not causal evidence.</p>',
        unsafe_allow_html=True
    )
...
with row2_col3:
    ...
    for _, row in sex_comparison_df.iterrows():
        sex = str(row["sex"])
        value = row["physical_inactivity_pct"]
        ...
        fig_sex.add_trace(
            go.Scatter(
                ...
                y=[sex, sex],
                mode="lines",
                line=dict(color="#d0d5dd", width=5),
                showlegend=False,
                hoverinfo="skip"
            )
        )
        ...
        fig_sex.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    size=16,
                    color=sex_color_map.get(sex, "#4C78A8")
                ),
                text=[f"{value:.1f}%"],
                textposition="middle right",
                name=sex,
                hovertemplate=f"{sex}: {value:.1f}%<extra></extra>"
            )
        )

    max_value = sex_comparison_df["physical_inactivity_pct"].max()
    ...
    st.markdown(
        '<p class="small-note">Rates by sex; not additive shares.</p>',
        unsafe_allow_html=True
    )
...
st.markdown(
    '<p class="small-note">Use the play button or year slider below the map to view changes from 2000 to 2022.</p>',
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    who are insufficiently physically active. The dashboard explores population-level associations and should not
